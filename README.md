...
Las consultas de negocio reconocidas, como `cuántas tareas tengo mañana para el dealer COMAZDCALC2`, van directamente al worker aunque no mencionen DynamoDB. `count-planned-tasks` cuenta tareas con `planned_start_at` dentro del día local del dealer, de todos los tipos, con la misma pertenencia al rango que `/planning`. Las consultas documentales conservan su ruta al brain. Fechas o filtros que el intérprete no admite producen una aclaración concreta; no se sustituyen silenciosamente por un conteo más amplio.

Configura en el `worker.env` privado `AWS_TASKS_TABLE` y `AWS_DEALER_CONFIG_TABLE` con los nombres físicos verificados del mismo entorno. No se adivinan tablas ni se consultan otros servicios AWS para resolverlas. El worker consulta `dealer_id-minimal-index` proyectando únicamente `dealer_id,time_zone`; exige una configuración única y una zona IANA válida. Si falta, devuelve el problema y no interpreta «mañana» usando una zona supuesta.

El conteo usa `Query` con `Select=COUNT` sobre `dealer_id-planned_start_at-index`, sin leer registros de tareas ni escanear la tabla. Recorre todas las páginas, incluidas las vacías con cursor, hasta agotar la consulta o su presupuesto de tiempo y páginas. Un límite o error antes de terminar produce un fallo explícito sin un total ni CSV parciales. Un resultado terminado informa fecha, zona horaria y consistencia eventual; excluye tareas sin hora planeada. La referencia temporal es la recepción original del mensaje, también cuando un posterior «dámelo en CSV» repite esa consulta. El CSV del conteo es un resumen de una fila.
