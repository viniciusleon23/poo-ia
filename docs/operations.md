...
Los avisos operativos de recepción (`ack`) y etapa (`progress`) se guardan fuera de memoria. Solo `response` y `error` cierran la ejecución de una solicitud. Un progreso suprimido tiene el envelope `completed_at` cerrado y sus partes no entregadas conservan `acked_at=NULL` y ningún ID remoto; esto indica descarte de un aviso obsoleto, no una entrega confirmada. La outbox omite esos envelopes en entrega y retención de pendientes. No hay migración adicional para esta mejora.

Los manifests del worker incorporan `phase` opcional (`prepare`, `edit`, `validate`, `document`, `publish`). Un manifest anterior carga con `null`. Preparado, cancelado y estados terminales limpian la fase. La aplicación muestra solo fases permitidas y emite seguimiento cada 60 segundos sin cambios; nunca incorpora stdout o prompts en esos avisos. Durante recuperación inicial se mantiene el arranque secuencial: una consulta directa recuperada lenta puede retrasar la conexión del bot, aunque los avisos permanezcan en SQLite.
